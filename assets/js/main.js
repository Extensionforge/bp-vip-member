jQuery( function( $ ) {
	if ( window.bpVerifiedMember ) {
		 ...
		 *
		 * @param {string} type Type of badge to init (vip, unvip)
		 */
	...
	$( 'button.bp-vip-member-request-button:not(.bp-vip-member-verification-pending)' )
		.on( 'click', function() {
			if ( requestLoading )
				return;
			...
			var nonce = $( this ).data( 'bp-vip-member-request-nonce' );
			var $this = $( this );
			...
			$.post( window.bpVerifiedMember.ajaxUrl, {
				action: 'bp_vip_member_request',
				nonce: nonce,
			}, function( result ) {
				if ( result.success ) {
					$this.html( result.data );
					$this.addClass( 'bp-vip-member-verification-pending' );
				}
			} );
		} );
} );